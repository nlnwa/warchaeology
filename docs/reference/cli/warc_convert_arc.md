...
```
  ...
  -h, --help                            help for arc
      --index-dir string                directory used to store index data (default "/home/vscode/.cache/warchaeology/arc")
  ...
  -K, --new-index                       start with a fresh index by deleting any existing index in --index-dir at startup
      --one-to-one                      write each input file to exactly one output file.
                                        Equivalent to: --concurrent-writers=1 --file-size=0 --name-generator=identity (default true)
  ...
  -r, --recursive                       walk input directories recursively
      ...
      --suffixes strings                only process files with these suffixes (default [.arc,.arc.gz])
